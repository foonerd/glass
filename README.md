...
The workspace version is **0.4.28**. The player reads the installed meter and spectrum FIFOs, rasters one frame, and uploads it when `DISPLAY` is set. `--headless` skips the window. `--output` writes a PNG or PPM. `--list`, `--theme`, `--meter`, `--interval`, `--fps`, `--threads` and `--snapshot` review the installed themes without touching the player's configuration; the wiki's Tests page has the workflow. Commits use [Conventional Commits](https://www.conventionalcommits.org/en/v1.0.0/). Versions use [Semantic Versioning](https://semver.org/spec/v2.0.0.html). The rules are in the wiki under Standards.
